...
+ (id)defaultPointClouds {
    static NSMutableArray *defaultPointClouds = nil;
    if (!defaultPointClouds) {
        defaultPointClouds = [self pointClouds];
    }
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    NSDictionary *dic = [defaults objectForKey:@"emoji3"];
    for (NSArray *dataArray in dic.allValues) {
        for (NSData *data in dataArray) {
            [defaultPointClouds addObject:[NSKeyedUnarchiver unarchiveObjectWithData:data]];
        }
    }
